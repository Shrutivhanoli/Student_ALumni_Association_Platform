<?php
$host = "localhost";
$username = "root"; // Change this if necessary
$password = ""; // Change this if necessary
$database = "student_db";

// Create connection
$conn = new mysqli($host, $username, $password, $database);

// Check connection
if ($conn->connect_error) {
    die("Connection failed: " . $conn->connect_error);
}

$sql = "SELECT * FROM stories";
$result = $conn->query($sql);
?>

<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>Alumni Success Stories</title>
    <style>
        .story-box {
            border: 1px solid #ccc;
            padding: 20px;
            margin-bottom: 20px;
            box-shadow: 0px 0px 10px rgba(0, 0, 0, 0.1);
            display: flex;
            align-items: flex-start;
            width: 80%;
            height: 250px;
            margin-left: 10%;
            box-shadow: 5px 7px 17px black;
            margin-top: 20px;

        }
        .story-image {
            height: 180px;
            margin-right: 20px;
            margin-top: 40px;
        }
        .story-details {
            flex: 1;
        }
        .story-title {
            font-size: 23px;
            margin-bottom: 10px;
        }
        .story-meta {
            margin-bottom: 15px;
        }
        .story-meta a {
            margin-right: 15px;
            text-decoration: none;
            color: #0073b1; /* LinkedIn Blue */
        }
        .story-skills{
            font-size: 20px;
            margin-left: 15px;
            margin-bottom: 20px;
        }
        .story-experience{
            font-size: 20px;
            margin-left: 15px;
            margin-bottom: 20px;
        }
        .story-date{
            font-size: 20px;
            margin-left: 15px;
            margin-bottom: 20px;
            position: relative;
            left: 580px;
            bottom: 170px;
            width: 160px;
        }
    </style>
</head>
<body>

    <div>
            <img src="bg_succcess.png" alt="img" style="height:400px;width:100%" >
            <img src="title.png" alt="title" style="height: 65px;position:relative;left:150px;bottom:387px">
            <img src="kitlogo.png" alt="image" style="height: 80px;position: absolute;left: 55px;top: 17px;">
            <h2 style="font-size:45px;color:white;position:absolute;bottom:470px;left:440px;color:orangered;">Alumni Success Stories</h2> 
            <a href="" style="position:absolute;top:20px;right:100px;padding-right:15px;color:black;text-decoration:none;font-size:25px; ">Home</a>
           <!-- <a href="upload_story.html" style="position:absolute;top:20px;right:45px;color:black;text-decoration:none;font-size:25px;" >Add</a>-->
    </div>

    <div style="height:auto;width:100%;background-color:white">
    <?php

    if ($result->num_rows > 0) {
        // Output each story as a box
        while($row = $result->fetch_assoc()) {
            echo "<div class='story-box'>";
            
            // Display image if available
            if ($row['image_path']) {
                echo "<img class='story-image' src='" . $row['image_path'] . "' alt='Alumni Image'>";
            } else {
                echo "<img class='story-image' src='placeholder.png' alt='No Image Available'>";
            }
            
            echo "<div class='story-details'>";
            echo "<div class='story-title'>" . $row['name'] . "</div>";
            echo "<div class='story-meta'>";
            
            // Display LinkedIn and Instagram links if available
            if ($row['linkedin']) {
                echo "<a href='" . $row['linkedin'] . "' target='_blank' style=font-size:17px>LinkedIn</a>";
            }
            if ($row['instagram']) {
                echo "<a href='" . $row['instagram'] . "' target='_blank' style=font-size:17px>Instagram</a>";
            }
            echo "</div>";
            echo "<div class='story-skills'><strong>Skills:</strong> " . $row['skills'] . "</div>";
            echo "<div class='story-experience'><strong>Experience:</strong> " . $row['experience'] . "</div>";
            echo "<div class='story-date'><strong>Date:</strong> " . $row['date'] . "</div>";
            echo "</div></div>";
        }
    } else {
        echo "<p>No success stories found.</p>";
    }
    $conn->close();
    ?>
    </div>


    <div id="footer" style=" background-color: rgb(20, 20, 20);height: 300px;width: 100%;margin-top: 50px;">
                  <hr style="background-color: white;height: 2px;position: relative;top: 30px;width: 80%;">
                  <pre><h3 style="color: white;font-family: 'times new roman';margin-top: 110px;margin-left: 20px;">Kolhapur Institute Of Technology,
Kolhapur</h3></pre>
                  </h3>
                  <img src="https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcRcyQiONPi2adI2TFdYD0sVDvMopYLoZzvNFQ&s" alt="image" style="height: 27px;border-radius: 50%;margin-left: 20px;">
                  <a href="https://www.facebook.com/official.kitcoek/"  style="color: white;font-size: 20px;margin-left: 5px;position: relative;top: -8px;text-decoration: none;">Facebook</a>
                  <img src="data:image/png;base64," alt="image"style="height: 27px;border-radius: 50%;margin-left: 20px;">
                  <a href="https://www.instagram.com/kitcoek.official/?hl=en" style="color: white;font-size: 20px;margin-left: 5px;position: relative;top: -8px;text-decoration: none;">Instrgram</a>
                  <img src="data:image/png;base64," alt="image" style="height: 27px;border-radius: 50%;margin-left: 20px;">
                  <a href="https://twitter.com/officialkitcoek?lang=en" style="color: white;font-size: 20px;margin-left: 5px;position: relative;top: -8px;text-decoration: none;">Twitter</a>
                  <img src="https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcSxdYwozA9MbvQoyUmsgHE9L-mHJ1Hxb-MlGg&s" alt="image" style="height: 27px;border-radius: 50%;margin-left: 20px;" >
                  <a href="https://www.youtube.com/@OfficialKITCoEK" style="color: white;font-size: 20px;margin-left: 5px;position: relative;top: -8px;text-decoration: none;">You Tude</a>
                </div>


</body>
</html>
